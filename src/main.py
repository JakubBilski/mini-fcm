...
if __name__ == "__main__":
    ...
    # for no_centers in range(10,35):
    #     for derivative_order in [0]:
    #         os.mkdir(pathlib.Path(f'./checkpoints/Cricket/mppi/{no_centers}_{derivative_order}/'))

    # for no_centers in range(10,35):
    #     for derivative_order in [0]:
    #         print(f"Learning no_centers {no_centers}, derivative_order {derivative_order}")
    #         generate_mppi_checkpoints(
    ...
    #     mppi_maps[i].train(xses_series[i])

    # for i in range(len(xses_series)):
    #     err = mppi_maps[i].get_error(xses_series[i])
    #     for j in range(len(xses_series)):
    #         err2 = mppi_maps[j].get_error(xses_series[i])
    #         if err2 < err:
    #             print(f"Model {j} was better than {i} in predicting series {i}")
    #     print(f"Error: {err}")
    #     mean += err
    # print(f"mean: {mean/len(xses_series)}")
    ...
    input_file = pathlib.Path('./data/Cricket/CRICKET_TEST.arff')
    xses_series, ys = loadArff.load_cricket_normalized(input_file)
    xses_series_derived = [derivatives.transform(xses_series, order) for order in [0]]
    plot_xs, plot_ys = [], []
    plot2_xs, plot2_ys = [], []
    plot3_xs, plot3_ys = [], []
    for no_centers in range(7, 21):
        for derivative_order in [0]:
            checkpoints_train_dir = pathlib.Path(f'./checkpoints/Cricket/mppi/{no_centers}_{derivative_order}/train')
            checkpoints_test_dir = pathlib.Path(f'./checkpoints/Cricket/mppi/{no_centers}_{derivative_order}/test')

            train_checkpoints = mppiCheckpoints.load_checkpoints(checkpoints_train_dir)
            test_checkpoints = mppiCheckpoints.load_checkpoints(checkpoints_test_dir)

            xses_series_transformed = xses_series_derived[derivative_order]
            xses_series_transformed = cmeans.transform(
                xses_series=xses_series_transformed,
                centers=train_checkpoints[0].cmeans_centers)

            xses_series_transformed = [xses_series_transformed[tp.input_data_index] for tp in test_checkpoints]

            train_models = [tp.mppi for tp in train_checkpoints]
            test_models = [tp.mppi for tp in test_checkpoints]
            ...
            err = sum([tp.mppi.get_error(xs) for tp, xs in zip(train_checkpoints, xses_series_transformed)])/len(train_checkpoints)
    ...
    fig, ax = plt.subplots()
    ax.plot(plot_xs, plot_ys, 'bo')
    ax.set(xlabel='train models auto-accuracy', ylabel='classification accuracy (rf)',
       title='mppi, Cricket, no derivatives')
    ax.grid()

    fig, ax = plt.subplots()
    ax.plot(plot2_xs, plot2_ys, 'bo')
    ax.set(xlabel='number of cmeans centers', ylabel='train models auto-accuracy',
       title='mppi, Cricket, no derivatives')
    ax.grid()
