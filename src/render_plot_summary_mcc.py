...
if __name__ == "__main__":
        ...
        for dataset in datasets:
            ...
            if args.best:
                ...
                for no_states in range(3, 8):
                    ...
                    mcc = np.mean([float(mcc) if mcc!='?' else 0.0 for mcc in state_df['mcc']])
                    if mcc > best_mcc:
                        best_mcc = mcc
                ...
                tested_datasets.append(dataset)
                mccs.append(np.mean([float(mcc) if mcc!='?' else 0.0 for mcc in dataset_df['mcc']]))
    ...
    if args.best:
        plt.suptitle('Matthews correlation coefficient for the best number of states')
    else:
        plt.suptitle('Mean Matthews correlation coefficient for number of states/centers 3-7')
    # plt.show()
